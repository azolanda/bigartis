...
$(document).ready(function () {
    ...
    prewsImg(imgs, '.item', '50vw', '350px');

    function prewsImg(prews, item, width, height) {
        prews.each(function () {
            let prew = $(this).closest(item);
            prew.css({
    ...
    onAfterChangeSlide('.slider', "data-slick-index", 0);

    function onAfterChangeSlide(sliderTarget, attrIndex, variant) {
        $(sliderTarget).on('afterChange', function () {
            let dataIndex = $('.slick-current').attr(attrIndex);

            let hrefAboutProject = $('.preview__heading-link')[parseInt(dataIndex)-variant].href;

            $('.slide__title')[0].innerHTML = "Заголовок: " + $('.preview__item p')[parseInt(dataIndex)-variant].innerHTML;
